...
export class WebCommons {
    page: Page;
    //constructor method to provide the tab details to perform the actions
    constructor(page:Page){
        this.page=page;
    }
    //commion method to generate the WebElement  from the liocator
    element(selector: string): Locator {
        return this.page.locator(selector);
    }

    //common methods to launch the application
    async launchApplication(url:string,expTitle?:string){
        await this.page.goto(url);
        if (expTitle && typeof expTitle === 'string'){            
